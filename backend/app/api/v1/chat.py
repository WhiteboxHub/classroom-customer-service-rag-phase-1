# classroom-customer-service-rag-phase-1/backend/app/api/v1/chat.py

import yaml
import os
from fastapi import APIRouter
from pydantic import BaseModel
from typing import List, Optional

from app.core.config import settings
from app.services.ingestion.language_detection import LanguageDetector

router = APIRouter()


class Message(BaseModel):
    role: str
    content: str


class ChatCompletionRequest(BaseModel):
    model: str
    messages: List[Message]
    stream: Optional[bool] = False


@router.get("/models")
async def list_models():
    models_file = "/resources/models.yaml"
    model_list = []

    def get_fallback_models():
        return [
            {
                "id": "gpt-4",
                "object": "model",
                "created": 1677610602,
                "owned_by": "openai",
            },
            {
                "id": "gpt-3.5-turbo",
                "object": "model",
                "created": 1677610602,
                "owned_by": "openai",
            },
        ]

    if os.path.exists(models_file):
        try:
            with open(models_file, "r") as f:
                data = yaml.safe_load(f)
                if "models" in data:
                    for m in data["models"]:
                        model_list.append(
                            {
                                "id": m["id"],
                                "object": "model",
                                "created": 1677610602,
                                "owned_by": m.get("provider", "system"),
                            }
                        )
        except Exception as e:
            print(f"Error reading models file: {e}")
            model_list = get_fallback_models()

    if not model_list:
        model_list = get_fallback_models()

    return {"object": "list", "data": model_list}


@router.post("/chat/completions")
async def chat_completions(request: ChatCompletionRequest):
    # 1. Extract latest user query
    user_query = request.messages[-1].content

    # 2. Detect query language (API boundary)
    language_detector = LanguageDetector()
    query_language = language_detector.detect_language(user_query)

    print(f"Detected query language: {query_language}")

    # 3. Derive search query using conversation history
    search_query = user_query
    if len(request.messages) > 1:
        last_exchange = (
            request.messages[-2].content
            if request.messages[-2].role == "assistant"
            else ""
        )
        if len(request.messages) > 2 and request.messages[-3].role == "user":
            last_exchange = request.messages[-3].content + " " + last_exchange

        if len(user_query.split()) < 15:
            for m in reversed(request.messages[:-1]):
                if m.role == "user":
                    search_query = f"{m.content} {user_query}"
                    break

    print(f"Processing query: {user_query}")
    print(f"Search query derived from history: {search_query}")

    # 4. Embed query
    from app.services.generation.embeddings import EmbeddingService

    embedder = EmbeddingService()
    query_vector = embedder.get_embedding(search_query)

    # 5. Retrieve context (optional language filtering)
    from app.services.retrieval.vector_store.milvus import MilvusClient

    vector_store = MilvusClient()

    try:
        retrieval_language = (
            query_language if settings.ENABLE_LANGUAGE_FILTERING else None
        )

        context_docs = await vector_store.search(
            query_vector,
            limit=3,
            language=retrieval_language,
        )
        context_text = "\n\n".join(context_docs)

        # from app.services.retrieval.language_aware_retriever import LanguageAwareRetriever

        # retriever = LanguageAwareRetriever()
        # context_docs = await retriever.retrieve(
        #     query_vector=query_vector,
        #     limit=3,
        #     language=query_language,
        # )
    except Exception as e:
        print(f"Retrieval failed: {e}")
        context_text = ""

    print(f"Retrieved context length: {len(context_text)}")

    # 6. Construct system prompt
    system_prompt = f"""
You are a helpful customer service assistant for Kaiser Permanente.

The user's query language is: {query_language}.
Respond in the same language unless explicitly requested otherwise.

Use the following context to answer the user's question.
If the answer is not in the context, say you don't know.

Context:
{context_text}
""".strip()

    # 7. Call LLM
    from openai import OpenAI

    api_key = os.getenv("OPENAI_API_KEY")
    base_url = os.getenv("OPENAI_API_BASE_URL")
    if base_url and "backend" in base_url:
        base_url = None

    client = OpenAI(api_key=api_key, base_url=base_url)

    messages = [{"role": "system", "content": system_prompt}]

    history_messages = [
        {"role": m.role, "content": m.content}
        for m in request.messages[:-1]
        if m.role in ["user", "assistant"]
    ]
    messages.extend(history_messages)
    messages.append({"role": "user", "content": user_query})

    try:
        response = client.chat.completions.create(
            model=request.model or "gpt-3.5-turbo",
            messages=messages,
            stream=False,
        )

        return {
            "id": response.id,
            "object": "chat.completion",
            "created": response.created,
            "model": response.model,
            "choices": [
                {
                    "index": 0,
                    "message": {
                        "role": "assistant",
                        "content": response.choices[0].message.content,
                    },
                    "finish_reason": "stop",
                }
            ],
            "usage": {
                "prompt_tokens": response.usage.prompt_tokens,
                "completion_tokens": response.usage.completion_tokens,
                "total_tokens": response.usage.total_tokens,
            },
        }
    except Exception as e:
        print(f"LLM call failed: {e}")
        return {
            "id": "error",
            "object": "chat.completion",
            "created": 0,
            "model": request.model,
            "choices": [
                {
                    "index": 0,
                    "message": {
                        "role": "assistant",
                        "content": f"I encountered an error processing your request: {str(e)}",
                    },
                    "finish_reason": "stop",
                }
            ],
        }
